...
export function EntryFormModal({ entry, open, onClose }: { entry: CashEntry | null; open: boolean; onClose: () => void }) {
  return (
    <Modal title={entry ? 'Modifica movimento' : 'Nuovo movimento'} open={open} onClose={onClose}>
...
function EntryForm({ entry, onDone }: { entry: CashEntry | null; onDone: () => void }) {
  ...
  async function submit(e: FormEvent, again = false) {
    ...
    if (parsed === null || parsed <= 0) {
      setFieldErrors({ amount: 'Inserisci un importo positivo' })
      return
    }
    if (categoryId === '') {
      setFieldErrors({ categoryId: 'Scegli una categoria' })
      return
    }
    setFieldErrors({})
    try {
  ...
  return (
    <form onSubmit={(e) => submit(e)} className="flex flex-col gap-4">
      ...
      <Field label="Descrizione (facoltativa)" error={fieldErrors.description}>
        {(id) => <input id={id} className="input" maxLength={500} value={description} onChange={(e) => setDescription(e.target.value)} />}
      </Field>
      <ErrorAlert message={error} />
      <div className="flex flex-wrap justify-end gap-2">
        {!entry && (
          <Button onClick={(e) => submit(e, true)} loading={save.isPending}>Salva e aggiungi un altro</Button>
        )}
